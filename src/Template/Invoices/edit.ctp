    ...
    <div class="col-md-12">
      ...
      <div class="box box-primary">
        ...
        <?= $this->Form->create($invoice, array('role' => 'form')) ?>
          <div class="box-body">
	    <div class="input select"><label for="client-id">Client</label><select name="client_id" id="client-id" disabled="true">
			<option value=""></option>
		...
		?>	
			...
			<option value="<?= $client->id?>" data-rate="<?=$client->default_rate?>" data-taxclass="<?=$client->tax_class_id?>" <?=$selected?>><?=$client->name?></option>
		<?php endforeach; ?>
	    </select></div>
          <?php	
...
<script>
  ...
  $(function () {
    var activities=<?=json_encode($invoice->activities)?>;
...
//	    "serverSide": true,       
	    ...
	    "type": "GET",
	    "data" : function ( d ) {  return {"client_id":"<?=$invoice->client_id?>" , "invoice_id":"<?=$invoice->id?>"}; },
	    "dataSrc" : "activities"
	    },
	    "columns": [
		    {
				...
				cost=qty*rate;
				if(JsonResultRow.invoice_id=="<?=$invoice->id?>"){
			    	return '<input type="checkbox" class="activity" name="activities['+JsonResultRow.id+']" value="'+JsonResultRow.id+'" data-rate="'+rate+'" data-cost="'+cost+'" checked>';
				}else{
